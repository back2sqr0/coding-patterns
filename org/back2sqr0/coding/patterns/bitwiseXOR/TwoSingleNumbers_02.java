...
/*
  Problem Statement: In a non-empty array of numbers, every number appears exactly twice except two numbers
...
This problem is quite similar to Single Number, the only difference is that, in this problem, we have two single numbers instead of one.
Can we still use XOR to solve this problem?
...
If a bit in n1xn2 is ‘1’, this means that num1 and num2 have different bits in that place, as we know that we can get ‘1’ only
when we do XOR of two different bits, i.e.,
...
public class TwoSingleNumbers_02 {
    ...
    public static void main(String[] args) {
        ...
        int[] result = TwoSingleNumbers_02.findSingleNumbers(arr);
        System.out.println("Single numbers are: " + result[0] + ", " + result[1]);

        arr = new int[] { 2, 1, 3, 2 };
        result = TwoSingleNumbers_02.findSingleNumbers(arr);
        System.out.println("Single numbers are: " + result[0] + ", " + result[1]);
    }
